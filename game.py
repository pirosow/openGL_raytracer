...
class App:
    ...
    def main(self):
        ...
        while running:
            ...
            self.updateCamera(self.eye, self.orientation, self.upVector, self.zoom, move=False)

            # refresh screen
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)

            self.ground.draw(self.model_location)
    ...
    def updateCamera(self, eye, orientation, upVector, zoom, move=True):
...
class Mesh: #fais que le mesh aie une position et eulers, et appelle le object
    def __init__(self, pos, eulers, dirPath, shader=None, size=1):
...
class Vehicle: #fais que le mesh aie une position et eulers, et appelle le object
    def __init__(self, pos, eulers, dirPath, shader=None, size=1, controls="airplane", acceleration=0.01, maxSpeed=10, control_offset=[0, 0, 0]):
        dirPath = os.path.join("models/", dirPath)
        ...
        self.control_offset = np.copy(np.radians(-np.array(eulers, dtype=np.float32)))

        self.maxSpeed = maxSpeed

        self.control = None

        if controls == "airplane":
            self.control = self.control_plane
    ...
    def control_plane(self):
        ...
        pitch = self.eulers[2] + self.control_offset[2]
        yaw = self.eulers[1] + self.control_offset[1]
    ...
    def move(self, forward):
        ...
        self.position += forward * self.velocity
